...
pub async fn handle_rejection(err:: Rejection) -> std::result::Result<impl Reply, Infallible> {
    let (code, message) = if.err_is_not_found() {
        ...
        match e {
            ...
            Error::JWTTokenCreationError => (StatusCode::UNAUTHORIZED, "Internal Server Error".to_string()),
            _=> (StatusCode::BAD_REQUEST, e.to_string()),

        }
    } else if err.find::<warp::rejection::MethodNotAllowed>().is_some() {
    (
